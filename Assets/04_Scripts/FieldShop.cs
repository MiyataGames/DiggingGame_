        ...
        else//売却の場合
        {
            ...
            for (int i = 0; i < party.Players[0].Items.Count; i++)//プレイヤーが持っているアイテム数だけくり返す
            {
                ...
                if (item.ItemBase.Id == selectedItem.ItemBase.Id)//もし選んだアイテムと売るアイテムが一致したら
                {
                    ...
                    int sellPrice = item.ItemBase.Price / 2 * quantity;//売却金＝選択したアイテムの購入価格の半値✖️個数
                    party.Players[0].gold += sellPrice;//現在の所持金＋売却金額
                    party.Players[0].Items[i].ItemCount -= quantity;//現在持っているアイテム数から売ったアイテム数の個数をひく
                    // 0個になったらUIを更新
            ...
            Debug.Log("数"+ quantityField.text );
            // Debug.Log("購入できない");
        }
